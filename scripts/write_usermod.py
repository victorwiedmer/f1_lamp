#!/usr/bin/env python3
"""Write F1LampUsermod.h with custom /f1 web interface."""
import os

# The HTML is embedded as a C++ raw string literal.
# All routes are registered on WLED's existing AsyncWebServer in setup().
  ...
  void registerRoutes() {
    ...
    server.on("/f1/wifi", HTTP_POST, [](AsyncWebServerRequest* req) {
      if (req->hasParam("ssid", true)) {
        ...
        s.toCharArray(clientSSID, sizeof(clientSSID));
        p.toCharArray(clientPass, sizeof(clientPass));
        doSerializeConfig();   // persist to flash
        doReboot = true;       // reconnect with new credentials
      }
      req->send(200, "text/plain", "ok");
    });
...
public:
  void setup() override {
    ...
    registerRoutes();
    _initDone = true;
    DEBUG_PRINTLN("[F1Lamp] Ready – UI at http://<IP>/f1");
  }
...
dest_paths = [
    r'C:\Users\Victor\source\repos\f1-lamp\usermod\F1LampUsermod.h',
    r'C:\Users\Victor\source\repos\f1-lamp\wled\usermods\F1LampUsermod\F1LampUsermod.h',
]
...
for path in dest_paths:
    ...
    print(f"Written {lines} lines ({kb}KB) to {path}")
